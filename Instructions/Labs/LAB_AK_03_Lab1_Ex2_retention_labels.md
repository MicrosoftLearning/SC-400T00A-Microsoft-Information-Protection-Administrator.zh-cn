---
ms.openlocfilehash: 8b7ef9af6f6003d79d7feee350c56ec5021b32ce
ms.sourcegitcommit: 8194831011a18168bab9873a95798c96a13cc448
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 05/13/2022
ms.locfileid: "145025192"
---
...
11. 在“查看并完成”页面上，选择“创建标签”按钮 。  在“已创建保留标签”页面上，选择“无操作”选项，然后选择“完成” 。

12. 返回到“标签”选项卡上的“数据生命周期管理”页面 。我们将在下一个练习发布标签。
...
14. 在“为保留标签命名”页上的“名称”、“针对用户的说明”和“针对管理员的说明”中，输入以下信息   ：
    ...
    - 针对管理员的说明：为保留期为 3 年的信用卡收据自动应用的保留标签。

15. 选择“下一步”按钮。

16. 对于“定义保留期设置”，请确保启用“将项保留特定时间段”设置 。 从下拉列表中选择“自定义”，并设置以下信息：
    - 保留期：3 年
    - 保留期开始依据：项创建时间
    - 保留期结束时：无操作
...
6. 如果可用，请选择“增值税申报表和支持性文档”，否则从现有策略中选择“一月后删除”（仅适用于本练习中的设置）。  注意：保留标签在发布后可能需要七天时间才能显示在 Exchange Online 中，且邮箱必须包含 10 MB 数据。
...
7.  如果可用，请将“文件夹策略”下拉列表设置为“增值税申报表和支持性文档”，否则从现有策略中选择“5 年后删除”（仅适用于本练习中的设置）  。 注意：保留标签在发布后可能需要七天时间才能显示在 Exchange Online 中，且邮箱必须包含 10 MB 数据
...
4. 在 SharePoint 登陆页面上，向下滚动并选择“通信网站”SharePoint 网站。 **注意**：在搜索栏中键入“通信站点”，然后进行搜索。
...
11. 如果该选项可用，请将“应用标签”设置为“增值税申报表和支持性文档”，并选择“保存”  。 由于发布保留标签可能需要一些时间，因此可能无法立即使用该选项，如果该选项不可用，请继续执行下一个任务。 稍后请随时返回到此任务，然后重试。
...
5. 对于突出显示的文档，选择三点的垂直的“...”按钮

6. 从菜单中，选择“详细信息”按钮。  

7. 侧菜单将出现在右侧。  你应会看到“应用标签”选项，单击该选项。

8. 如果该选项可用，请将“应用保留标签”设置为“增值税申报表和支持性文档” 。 由于发布保留标签可能需要一些时间，因此可能无法立即使用该选项，如果该选项不可用，请继续执行下一个任务。 稍后请随时返回到此任务，然后重试。
