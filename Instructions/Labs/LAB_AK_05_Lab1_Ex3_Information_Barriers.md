...
1. 验证 Microsoft Teams 中是否启用了按名称搜索
1. 将组织中的用户分群
1. 创建信息屏障策略
1. 应用信息屏障策略
...
1. 在终端窗口中，输入 cmdlet 以连接到安全与合规性 PowerShell，

    ````powershell
    Connect-IPPSSession
    ````

    以 **Joni Sherman**`JoniS@WWLxZZZZZZ.onmicrosoft.com`（其中 ZZZZZZ 是实验室托管提供程序提供的唯一租户 ID）的身份登录。 Joni 的密码是在上一练习中设置的。
